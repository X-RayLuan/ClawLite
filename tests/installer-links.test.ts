...
test('installer links keep macOS on the previous line and move Windows to v1.3.140', () => {
  assert.equal(INSTALLER_VERSION.windows, 'v1.3.140')
  assert.equal(INSTALLER_VERSION.macos, 'v1.3.137')
  assert.equal(
    WIN_INSTALLER_URL,
    'https://github.com/X-RayLuan/ClawLite-Brand-Installer/releases/download/v1.3.140/clawlite-setup.exe'
  )
  assert.equal(
    MAC_INSTALLER_URL,
