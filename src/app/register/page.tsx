...
import { getSafeReturnTo } from "@/lib/auth-flow";
...
export default function RegisterPage() {
  ...
  useEffect(() => {
    ...
    async function completeLogin(session: { user: { id: string; email?: string | null } }) {
      if (redirectingRef.current) return;
      redirectingRef.current = true;
      await syncPostLoginClientState({ supabase: client as any, user: session.user, returnTo });
  ...
  async function sendCode(emailValue: string) {
    ...
    const returnTo = getSafeReturnTo(new URLSearchParams(window.location.search).get("returnTo")) || "/clawrouter/dashboard";
  ...
  async function onSubmitCode(e: FormEvent) {
    ...
    const returnTo = (() => {
      try { return window.localStorage.getItem("clawlite-post-login-returnTo") || "/clawrouter/dashboard"; } catch { return "/clawrouter/dashboard"; }
    })();
  ...
  return (
    <main className="mx-auto min-h-[70vh] max-w-2xl px-6 py-16">
      ...
      <p className="mt-3 text-ink/70">
        Enter your email and verify with a code to get started. No password needed.
      </p>
      ...
      <form onSubmit={step === "email" ? onSubmitEmail : onSubmitCode} className="mt-4 rounded-2xl border border-black/10 bg-white p-6 shadow-soft">
        ...
        {step === "code" ? (
          <>
            <label className="mt-5 block text-sm font-medium text-ink">6-digit code</label>
                ...
                <input
                  ...
                  onKeyDown={(e) => {
                    if (e.key === "Backspace") {
                      ...
                      if (code[i]) { chars[i] = ""; setCode(chars.join("")); }
                      else if (i > 0) { chars[i - 1] = ""; setCode(chars.join("")); const inputs = document.querySelectorAll<HTMLInputElement>(".otp-box"); inputs[i - 1].focus(); }
                    }
                  }}
                  onPaste={(e) => {
                    e.preventDefault();
            ...
            <div className="mt-4 flex items-center justify-between gap-3 text-sm">
              <button type="button" onClick={() => { setStep("email"); setCode(""); setError(""); setMessage(""); }} className="text-ink/65 underline-offset-4 hover:text-ink hover:underline">
                Change email
              </button>
              <button type="button" onClick={() => { if (loading || resendCooldown > 0) return; void sendCode(sentEmail); }} disabled={loading || resendCooldown > 0} className="text-ink/65 underline-offset-4 hover:text-ink hover:underline disabled:cursor-not-allowed disabled:text-ink/35">
