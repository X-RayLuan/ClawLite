    ...
    <main className="gradient-bg min-h-screen flex items-center justify-center px-4">
      <div className="w-full max-w-md">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="font-display text-2xl font-semibold text-stone-900">管理员登录</h1>
          <p className="mt-1 text-sm text-stone-500">
            {step === 'email' && '请输入管理员邮箱以继续'}
            {step === 'verify' && `验证码已发送至 ${sentEmail}`}
            {step === 'error' && '出错了，请重试'}
          </p>
        </div>
          ...
          <CardContent className="p-6 sm:p-8 space-y-6">
            ...
            {isLoading && (
              <div className="absolute inset-0 z-10 flex items-center justify-center rounded-[2.2rem] bg-white/70 backdrop-blur-sm">
                <div className="flex flex-col items-center gap-3">
                  ...
                  <span className="text-sm text-stone-500">处理中…</span>
                </div>
              </div>
            )}
            ...
            {step === 'email' && (
              <form onSubmit={handleSendCode} className="space-y-4">
                <div>
                  <label className="mb-1.5 block text-xs font-semibold text-stone-600">邮箱地址</label>
                  <Input
                    type="email"
                    value={email}
            ...
            {step === 'verify' && (
              <form onSubmit={handleLogin} className="space-y-6">
                ...
                <div className="flex items-center justify-between text-sm">
                  ...
                  >
                    {resendCooldown > 0 ? `${resendCooldown}s 后重新发送` : '重新发送'}
                  </button>
                </div>
              </form>
            )}
