...
async function recordUsage(params: {
  ...
  try {
    ...
    if (insertError) {
      console.error("[openai/v1/responses] usage insert error:", insertError);
      return { success: false, error: insertError.message };
    }
    return { success: true };
  } catch (err) {
    console.error("[openai/v1/responses] failed to record usage:", err);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.error("[openai/v1/responses] balance check failed:", err);
  ...
  const maxTokens = body?.max_tokens || body?.maxOutputTokens || 4096;

  // Extract input text from OpenAI Responses API format
  // The Responses API uses "model" + "input" (either messages array or text/array of content parts)
  let inputText = "";
  if (body?.input) {
    if (typeof body.input === "string") {
      inputText = body.input;
    } else if (Array.isArray(body.input)) {
      // Could be messages or content array
      const messages = body.input.find((item: any) => Array.isArray(item.messages || item.parts));
      if (messages) {
        inputText = JSON.stringify(messages.messages || messages.parts);
      } else {
        inputText = JSON.stringify(body.input);
      }
    }
  } else if (body?.messages) {
    // Fallback to chat completions style
    inputText = JSON.stringify(body.messages);
  }
  ...
  try {
    const freezeTx = await freezeBalance(
      ...
      estimatedCost,
      undefined,
      `openai_v1_responses:${model}`,
    );
    freezeTxId = freezeTx.id;
  } catch (err) {
    console.error("[openai/v1/responses] freeze failed:", err);
  ...
  try {
    // Proxy to ezrouter's responses endpoint (same path)
    const ezrouterResponse = await fetch(`${ezrouterBaseUrl}/api/openai/v1/responses`, {
      method: "POST",
      headers: {
    ...
    if (!ezrouterError) {
      ...
      try {
        ...
        console.error("[openai/v1/responses] chargeBalance failed:", err);
      }
    }
    ...
    if (!usageResult.success) {
      console.error("[openai/v1/responses] usage recording failed:", usageResult.error);
    }
  }
}
