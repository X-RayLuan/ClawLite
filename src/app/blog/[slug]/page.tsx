...
import { blogStaticParams, getPost } from '@/lib/blog';
...
function buildFaqSchema(post: BlogPostData) {
...
function renderContent(content: string) {
  ...
  lines.forEach((line, index) => {
    ...
    // Skip H1 headings (title is already rendered in <header>)
    if (trimmed.startsWith('# ') && !trimmed.startsWith('## ')) {
      return;
    }

    // Skip meta description lines
    if (trimmed.startsWith('**Meta description:**') || trimmed.startsWith('**Meta description:')) {
      return;
    }

    // Skip "FAQ Schema" section heading
    if (trimmed === '## FAQ Schema') {
      return;
    }
